...
当前 `investment_data` 项目负责从 MySQL 导出 A 股日线数据，转换为 Qlib binary provider 格式，并发布为 GitHub Release 资产 `qlib_bin.tar.gz`。为了让使用者可以在本地直接查询离线行情，需要新增一个面向终端的 CLI 工具。
...
```bash
investment-data kline --symbol SH600519 --start 2024-01-01 --end 2024-12-31
```
...
```bash
investment-data kline --symbol SH600519 --start 2024-01-01 --end 2024-12-31 --format json
```

### 3.4 指定字段

```bash
investment-data kline \
  --symbol SH600519 \
  --start 2024-01-01 \
  --end 2024-12-31 \
...
```bash
investment-data --provider-uri ~/.qlib/qlib_data/cn_data kline --symbol SH600519
```

也支持环境变量：

```bash
export INVESTMENT_DATA_PROVIDER_URI=~/.qlib/qlib_data/cn_data
```
...
```toml
[project.scripts]
investment-data = "investment_data_cli.main:main"
```
...
```bash
investment-data download [options]
```
...
5. 解压到临时 staging 目录。
6. 校验 staging 目录符合 Qlib provider 基本结构。
7. 原子替换目标目录。
8. 写入本地元数据文件 `.investment_data_meta.json`。
...
```bash
investment-data update [options]
```
...
3. 如果本地 `release_tag` 与远端相同，默认跳过。
4. 如果远端更新，执行 download 流程。
5. 支持 `--force` 忽略版本判断并重新安装。
...
| 参数 | 默认值 | 说明 |
| --- | --- | --- |
| `--symbol` | 必填 | Qlib symbol，例如 `SH600519`、`SZ000001` |
...
```bash
investment-data symbols
investment-data symbols --prefix SH60
investment-data symbols --contains 600519
```
...
```bash
investment-data calendar --start 2024-01-01 --end 2024-01-31
```
...
3. 校验 staging。
4. 将当前 `cn_data` rename 为 backup。
5. 将 staging rename 为 `cn_data`。
6. 成功后删除 backup；失败则回滚。
...
当前导出脚本中 `stock_basic.qlib_code` 是最终 Qlib symbol。CLI 不应自行转换证券代码，只接受 provider 中存在的 symbol。
...
| `SH600519` | `SH600519` |
| `sh600519` | `SH600519` |
| `600519.SH` | 可选转换为 `SH600519` |
| `000001.SZ` | 可选转换为 `SZ000001` |
...
```bash
investment-data --dataset cn_stock_1d kline --symbol SH600519
investment-data --dataset us_stock_1d kline --symbol AAPL
investment-data --dataset cn_etf_1d kline --symbol SH510300
```
...
| 数据目录不存在 | `Provider directory does not exist. Run: investment-data download` |
...
```bash
python -m investment_data_cli.main download --target-dir /tmp/qlib_data/cn_data
python -m investment_data_cli.main info --provider-uri /tmp/qlib_data/cn_data
python -m investment_data_cli.main kline --provider-uri /tmp/qlib_data/cn_data --symbol SH600519 --start 2024-01-01 --end 2024-01-10
python -m investment_data_cli.main update --provider-uri /tmp/qlib_data/cn_data
```
...
CLI 将 tag 作为数据版本。后续如果同一天有多次发布，可以扩展为：
...
或继续通过同 tag 覆盖 asset，但本地 update 需要额外比较 asset size / updated_at。

## 14. 实施计划

### Phase 1：最小可用版本

1. 新增 Python 包 `investment_data_cli`。
...
1. Qlib symbol 是否统一使用 `SH600519` / `SZ000001`，还是需要兼容 `600519.SH`？
2. `qlib_bin.tar.gz` 解压后根目录是 provider 内容本身，还是包含一层 `qlib_bin/` 目录？实现时需要兼容或明确约定。
3. Release latest 是否总是指向最新可用交易日？如果同一天重发 asset，update 是否需要比较 asset `updated_at`？
4. 是否需要支持断点续传？首版可不支持。
5. 是否需要把 CLI 放在当前 exporter 仓库内，还是新建独立包？首版建议放在当前仓库内，复用 Release 与文档。
...
investment-data download
investment-data info
investment-data kline --symbol SH600519 --start 2024-01-01 --end 2024-01-10 --format csv
investment-data update
```
